...
1、修改两个数据文件的优先读取目录
   实时行情数据代码文件 stock_codes.conf 优先从exe运行目录读取，便于随时更新，如读取失败从原路径读取。
   财务数据文件financial_data.json也是优先从exe运行目录下的stock_data读取，便于更新，如读取失败从exe解压的临时目录下的stock_data读取。
2、修正实时运行中stock_codes.conf读取偶发失败的问题，不再每次引用时读取，改为初始化时全局设置。
3、收益率和资产重置
   之前版本每次重启交易后，不读取历史性能数据（只有持仓审视读取了历史持仓数据），导致重启后资产和收益率都会重置为初始值，现修改每次重启后读取数据库中的历史性能数据，以维持资产和收益率的连续性（web页面和UI页面的性能数据曲线，暂不显示历史收益率，只从重启时点显示，但会显示“连续化”后的数据，之前版本显示的是完全重置后的新数据）。如需完全重置，可删除log文件夹下的所有文件。
4、UI页面部分按钮重命名，比如“生成股票池”改成“下载股票列表”，下载并生成全市场的股票名单。
5、exe文件减少体积，程序初始化时需要加载大量数据，启动时需要数十秒时间，减重以适当提速。
...
7、修复UI页面生成股票池时tdqm找不到输出流的问题（exe版本短期内无需使用生成股票池功能，全市场股票池文件已经打包在rar中）
